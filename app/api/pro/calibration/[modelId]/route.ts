...
/**
 * Paid proxy for a model's calibration and known-unknowns record.
 *
 ...
 * calling /api/models/:id/calibration directly from the internet gets a 401
...
const MINIMUM: Plan = REQUIRED_PLAN.calibration;
...
export async function GET(
  ...
  if (!token) {
    console.error('[pro/calibration] PRO_API_TOKEN is not configured');
    return NextResponse.json(
  ...
  try {
    const upstream = await fetch(`${apiBase}/api/models/${modelId}/calibration`, {
    ...
    console.error('[pro/calibration] Upstream request failed:', error);
    return NextResponse.json(
      { success: false, error: 'Could not load calibration data' },
      { status: 502 }
    );
  }
}
